---
version: alpha
name: Canal-Plus-blue-data-infrastructure-console
description: Canal Plus uses the referenced blue-white institutional system as a starting point, but adapts it into a developer and DBA control console for data synchronization, migration, validation, and node operations. The product surface should stay light, compact, and operational: pure white cards, very pale blue-gray page canvas, `#0052ff` for primary actions and selected states, restrained borders, low or no shadow, dense readable tables, clear status tags, and dark monospace log consoles only where logs are the primary content.
design_direction: Blue Data Infrastructure Console
avoid: Financial exchange, wallet, crypto, marketing landing-page, heavy admin dashboard, glassmorphism, decorative gradients, large shadows.
