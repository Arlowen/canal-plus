    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
      <section className="rounded-xl border border-line bg-white shadow-panel">
        <div className="flex flex-col gap-3 border-b border-line p-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button onClick={createNew} className="inline-flex items-center justify-center gap-2 rounded-lg bg-coal px-3 py-2 text-sm text-white transition active:scale-[0.98]">
            <Plus size={16} />
            新增规则
          </button>
        </div>
        ...
        <form onSubmit={submit} className="rounded-xl border border-line bg-white p-5 shadow-panel">
          ...
          <div className="mt-4 grid gap-4">
            ...
            <SettingsField label="作用范围">
              <select className="control" value={form.taskId || ""} onChange={(event) => setForm({ ...form, taskId: event.target.value })}>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <SettingsField label="延迟阈值秒">
                <input className="control" type="number" min={1} value={form.delayThresholdSeconds} onChange={(event) => setForm({ ...form, delayThresholdSeconds: Number(event.target.value) })} />
              </SettingsField>
              <SettingsField label="错误次数阈值">
                <input className="control" type="number" min={0} value={form.errorThreshold} onChange={(event) => setForm({ ...form, errorThreshold: Number(event.target.value) })} />
              </SettingsField>
            </div>
            <SettingsField label="Webhook">
              <input className="control" value={form.webhookUrl || ""} onChange={(event) => setForm({ ...form, webhookUrl: event.target.value })} placeholder="https://example.com/webhook" />
            </SettingsField>
            <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
              <button className="inline-flex items-center justify-center gap-2 rounded-lg bg-coal px-3 py-2 text-sm text-white transition active:scale-[0.98]">
                保存规则
              </button>
              <button
                type="button"
